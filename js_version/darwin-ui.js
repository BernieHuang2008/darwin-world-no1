...
const shseedBtn = document.getElementById('shseed-btn');
...
function initializeEventListeners() {
    ...
    seedBtn.addEventListener('click', () => {
        const newseed = prompt("Enter a seed value:");
        if (newseed) {
            seed = newseed;
        }
    });

    shseedBtn.addEventListener('click', () => {
        alert(`Current seed value: ${seed}`);
    });
 ...
 */
function nextRound() {
    if (currentStep < roundHistory.length - 1) {
        currentStep++;
        displayRound(currentStep);
        return;
    }
...
function clearLivesIfDead() {
    const lives = document.querySelectorAll('.life');
    lives.forEach(life => {
        const name = life.id.replace('life_', '#');
        if (!conditionCards[name]) {
            life.remove();
            delete conditionCards[name];
